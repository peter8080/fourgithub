...
//步骤2 定义一个变量 pthread_t voiceThread 的由来
...
//步骤一 线程函数 返回的是无类型指针（无类型的参数）
//步骤二 调用init
//步骤三 把c_fd接过来，因为客户端要保持不断的接入。 故有c_fd的初始化之类的由来
//步骤四 启动线程        pthread_create的由来
//步骤五 读线程 pthread_t readThread的由来 放到pthread_create里面去
//步骤六 创建一个读的线程函数 void *read_thread(void*datas)的由来
//步骤七 把变量放全局 intc_fd放在了开头 *socketHandler也是的
//步骤八 因为有read 所以加上头文件
...
void *socket_thread(void* datas)
{
	...
	while(1){
		...
		pthread_create(&readThread,NULL, read_thread,NULL);//创建新线程去对接
	}
